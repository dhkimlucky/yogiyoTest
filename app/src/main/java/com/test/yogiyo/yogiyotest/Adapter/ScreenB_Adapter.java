...
public class ScreenB_Adapter extends BaseAdapter{
    ...
    public ScreenB_Adapter(Activity activity, List<UserData> itemList) {
        ...
        viewHolder.like_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(item.isLike()){
                    ...
                    // userDataList에서 unlike로 된 아이템의 속성을 변경
                    if(App.getInstance().getUserData() != null){
                        userData = App.getInstance().getUserData();
                        for(int i = 0 ;i < userData.size(); i++){
                            if(userData.get(i).getId() == item.getId()){
                                userData.get(i).setLike(false);
                                break;
                            }
                        }
                        // likedUserList 에서 unlike 된 아이템을 삭제
